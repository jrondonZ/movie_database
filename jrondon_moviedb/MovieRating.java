...
public class MovieRating implements Serializable{
	...
	// Move the new node up the tree as needed to restore the heap property
		int i = n - 1;
		int parent = (i - 1) / 2;
		while (i > 0 && s[i].getPopularityScore() <= s[parent].getPopularityScore()) {
			swap(i, parent);
			i = parent;
			parent = (i - 1) / 2;
		}
 ...
 */
	...
	public MovieNode deleteMin(){
		...
		while (i < n) {
			...
			if (left < n && s[left].getPopularityScore() <= s[min].getPopularityScore()) {
				min = left;
			}
	
			if (right < n && s[right].getPopularityScore() <= s[min].getPopularityScore()) {
				min = right;
			}
